...
```
```{python}
df_unique["price_pcm"].describe()
```
```{python}
df_price_bins = df_unique.copy()

# Create custom width bins (every 250)
bin_width = 250
...
#labels = [f'{bins[i]}-{bins[i+1]+bin_width-1}' for i in range(len(bins))]
labels = []
...
df_bin_counts = df_price_bins.groupby('price_bin', observed=False).size().reset_index(name='count')
df_bin_counts
```
